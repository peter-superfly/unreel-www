...
export const SubscribeToBlocks = gql`
  subscription SubscribeToAccountBlocks($owner_uid: uuid!) {
    blocks(
      where: { owner_uid: { _eq: $owner_uid } }
      order_by: { page_row_idx: asc }
    ) {
      text
      uid
      type
...
export const UpdateAccount = gql`
  mutation UpdateAccount(
    ...
    update_accounts(
      where: { uid: { _eq: $uid } }
      _set: { email: $email, fullname: $fullname, title: $title, bio: $bio }
    ) {
      affected_rows
      returning {
        uid
...
export const UpdateUsername = gql`
  mutation UpdateAccount($uid: uuid!, $username: String!) {
    update_accounts(
      where: { uid: { _eq: $uid } }
...
export const IncrementBlockRowIdx = gql`
  mutation IncrementBlockRowIdx($owner_uid: uuid!, $page_row_idx: Int!) {
    update_blocks(
      where: {
...
export const UpdateBlockIndex = gql`
  mutation SetBlockRowIdx($uid: uuid!, $page_row_idx: Int!) {
    update_blocks_by_pk(
      pk_columns: { uid: $uid }
      _set: { page_row_idx: $page_row_idx }
    ) {
      page_row_idx
    }
  }
...
export const EditBlockText = gql`
  mutation EditBlockText($uid: uuid!, $text: String!) {
    update_blocks_by_pk(pk_columns: { uid: $uid }, _set: { text: $text }) {
      page_row_idx
    }
  }
`;
